  			...
  			<div class="inner-cont">
  				...
  				<div class="detail-cont">
  					<div class="box">
  						...
  						<input type="text"placeholder="请输入登录账号" v-model="username" disabled/>
  					</div>
  					<div class="box">
  					...
  					<div class="box">
  						...
  						<label v-for="item,index in type" @click="chooseTypeFn(index)">
...
<script>
  ...
  export default{
    name:'SystemEditMember.vue',
    data(){
      return{
        username:this.$store.state.editMemberData.username,
        password:this.$store.state.editMemberData.password,
        loginname:this.$store.state.editMemberData.displayname,
        oldPassword:this.$store.state.editMemberData.password,
        reword:[
          {name:'内置部件栏目',checked:false},
          {name:'复制账号栏目',checked:false}
        ],
        // beizhu:this.$store.state.editMemberData.beizhu,
        beizhu:'',
        state:null//启用禁用状态
      }
    },
    computed:{
      ...
      userEditUrl(){
        return this.$store.state.userEditUrl
      },
      editMemberData(){
        return this.$store.state.editMemberData
      },
      type(){
        if(this.editMemberData.state==0){
          var obj = [
            {name:'启用',checked:true},
            {name:'禁用',checked:false}
          ]
        }else{
          var obj = [
            {name:'启用',checked:false},
            {name:'禁用',checked:true}
          ]
        }
        return obj
      }
    ...
    methods:{
      ...
      chooseTypeFn(i){
        ...
        this.$set(this.type[i],'checked',true);
      ...
      addMemberFn(){
        var This = this;
        if(this.username==''||this.password==''||this.loginname==''){
          this.$layer.alert('请填写完整信息！')
        }else if(this.type[0].checked==false&&this.type[1].checked==false){
          this.$layer.alert('请选择状态！')
        }else{
          if(this.oldPassword!=this.password){
            var newEditUrl = this.staticUrl+this.userEditUrl+'?username='+this.username+'&displayname='+this.loginname+'&password='+this.password+'&beizhu='+this.beizhu+'&state='+this.state;
          }else{
            var newEditUrl = this.staticUrl+this.userEditUrl+'?username='+this.username+'&displayname='+this.loginname+'&beizhu='+this.beizhu+'&state='+this.state;
          }
          axios.get(newEditUrl).then(function (res) {
            console.log(res);
            if(res.data.error==0){
