    ...
    <div class="user-mdoal-container type-short modal-add">
      ...
      <div class="user-modal-item-cont">
        <div class="user-modal-item">
          ...
          <span>:</span>
          <input type="" placeholder="请输入名称" name="" id="" value="" v-model="addName" />
        </div>
        <div class="user-modal-item">
  ...
  export default{
    ...
    methods:{
      ...
      subMitFn(){
        var that = this;
        var currentUrl = ''
        if(this.addType=='user'){  //用户

        }else if(this.addType=='aprtment'){  //部门
          if(this.addName!=''){
            currentUrl = this.staticUrl + this.systemDepartUrl + '?id=0&departname='+this.addName +'&up_id=0';
        ...
        }else if(this.addType=='groups'){ //小组
          if(this.addName!=''){
            currentUrl = this.staticUrl + this.systemDepartUrl + '?id=0&departname='+this.addName +'&up_id='+this.addId;
            axios.get(currentUrl).then(function(res){
              if(res.data.error==0){
                that.$layer.alert('添加成功！')
                that.colseModelAdd();
